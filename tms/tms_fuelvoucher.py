...
class tms_fuelvoucher(osv.osv):
    ...
    _columns = {
        ...
        #'office_id'       : fields.related('travel_id', 'office_id', type='many2one', relation='tms.office', string='Office', store=True, readonly=True),
        'office_id'       : fields.many2one('tms.office', 'Office', required=True, states={'cancel':[('readonly',True)], 'confirmed':[('readonly',True)],'closed':[('readonly',True)]}),
        ...
        'picking_id_cancel' : fields.many2one('stock.picking', 'Stock Picking Return', required=False, readonly=True, ondelete='restrict'),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        office_id = 'office_id' in vals and vals['office_id'] or False
        #print "vals: ", vals
        if 'travel_id' in vals and vals['travel_id']:
            travel = self.pool.get('tms.travel').browse(cr, uid, vals['travel_id'])
            office_id = travel.office_id.id
            vals.update({'office_id': travel.office_id.id, 'unit_id': travel.unit_id.id})
        #print "office_id: ", office_id
        supplier_seq_id = self.pool.get('tms.office.fuel.supplier.seq').search(cr, uid, [('office_id', '=', office_id),('partner_id', '=', vals['partner_id'])])
        if supplier_seq_id:
            seq_id = self.pool.get('tms.office.fuel.supplier.seq').browse(cr, uid, supplier_seq_id)[0].fuel_sequence.id
        else:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for rec in self.browse(cr, uid, ids):
            'office_id' in vals and not rec.no_travel and vals.pop('office_id')
    ...
    def create_picking(self, cr, uid, fuelvoucher, picking_type, source_picking_id, context=None):
        ...
        picking = {
                ...
                'note'              : _('Record created from Fuelvoucher %s %s. Travel: %s') % 
                                            (fuelvoucher.name, (picking_type =='out' and 'Confirmation' or 'Cancelation'),
                                             fuelvoucher.travel_id.name or _('No Travel')),            
                'move_lines'        : [move],
                'move_type'         : 'direct',
                #'type'              : 'internal',
                #'invoice_state'     : 'none',
        ...
        for fuelvoucher in self.browse(cr, uid, ids, context=context):
            if fuelvoucher.product_uom_qty <= 0.0:
                ...
                move_line = (0,0, {
                                    ...
                                    'office_id'  : fuelvoucher.office_id.id,
                                    'product_id'    : fuelvoucher.product_id.id,
