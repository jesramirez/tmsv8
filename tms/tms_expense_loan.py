...
- Percent: Discount will be a percentage of total Loan Amount
        ...
        'office_id'       : fields.related('employee_id', 'office_id', type='many2one', relation='tms.office', string='Office', store=True, readonly=True),
        ...
        'manually_closed': fields.boolean('Manually closed', readonly=True)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'employee_id' in vals and vals['employee_id']:
            ...
            seq_id = employee.office_id.tms_loan_seq.id
            if seq_id:
                seq_number = self.pool.get('ir.sequence').get_id(cr, uid, seq_id)
                values['name'] = seq_number
            else:
                raise osv.except_osv(_('Loan Sequence Error !'), _('You have not defined Loan Sequence for shop ' + employee.office_id.name))
    ...
    def get_loan_discounts(self, cr, uid, employee_id, expense_id, context=None):
        ...
        if len(res):
            ...
            for x in expense_obj.browse(cr, uid, [expense_id])[0].expense_line:
                if x.loan_id.id:
                    if not x.loan_id.manually_closed:
