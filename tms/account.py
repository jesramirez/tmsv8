...
class account_account(osv.osv):
    ...
    _columns = {
        ...
        'tms_office_mandatory'    : fields.boolean('TMS Office Mandatory', help= 'If set to True then it will require to add Office to Move Line'),
        }

    _defaults = {
        'tms_vehicle_mandatory' : lambda *a :False,
        'tms_employee_mandatory' : lambda *a :False,
        'tms_office_mandatory' : lambda *a :False,
        }
...
class account_move_line(osv.osv):
    ...
    _columns = {
        ...
        'office_id': fields.many2one('tms.office', 'Office', required=False),
        }
    ...
    def _check_mandatory_office(self, cr, uid, ids, context=None):
        for record in self.browse(cr, uid, ids, context=context):
            return (record.account_id.tms_office_mandatory and record.office_id.id) if record.account_id.tms_office_mandatory else True
        return True
    ...
    _constraints = [
        ...
        (_check_mandatory_office, 'Error ! You have not added Office to Move Line', ['office_id']),
        ]
