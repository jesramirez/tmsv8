...
class account_invoice(osv.osv):
    ...
    _columns = {
        ...
        'product'       : fields.function(_get_waybill_info, string='Product',  type='char',  multi=True, method=True),
        ...
        'supplier_waybill_ids': fields.one2many('tms.waybill', 'supplier_invoice_id', 'Waybills from Supplier', readonly=True, required=False),
    }
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context):
            ...
            if invoice.state in ('draft', 'cancel') and invoice.internal_number != False and invoice.origin == 'TMS-Fuel Voucher':
                self.write(cr, uid, [invoice.id], {'internal_number':False})
            if invoice.type=='out_invoice':
                for waybill in invoice.waybill_ids: rec_ids.append(waybill.id)
                xtype = 'waybills'
            elif invoice.type=='in_invoice' and invoice.supplier_waybill_ids:
                for waybill in invoice.supplier_waybill_ids: rec_ids.append(waybill.id)                    
                xtype = 'supplier_waybills'
            elif invoice.type=='in_invoice' and invoice.fuelvoucher_ids:
                for fuelvoucher in invoice.fuelvoucher_ids: rec_ids.append(fuelvoucher.id)
                xtype = 'fuelvouchers'

            if invoice.type=='out_invoice' and rec_ids and xtype=='waybills': # Factura de cliente resultado de Cartas Porte
                self.pool.get('tms.waybill').write(cr, uid, rec_ids, {'invoice_id': False, 'invoiced': False, 'invoice_paid':False, 'invoice_name':False})
            elif invoice.type=='in_invoice' and rec_ids and xtype=='supplier_waybills': # Factura de proveedor resultado de Cartas Porte
                self.pool.get('tms.waybill').write(cr, uid, rec_ids, {'supplier_invoice_id': False, 'supplier_invoiced': False, 'supplier_invoice_paid':False, 'supplier_invoice_name':False})            
            elif invoice.type=='in_invoice' and rec_ids and xtype=='fuelvouchers': # Factura de proveedor resultado de Vales de Combustible
                self.pool.get('tms.fuelvoucher').write(cr, uid, rec_ids, {'invoiced': False, 'invoice_paid':False, 'invoice_name':False})
...
class account_invoice_line(osv.osv):
    ...
    _columns = {
            'vehicle_id'    : fields.many2one('fleet.vehicle', 'Vehicle', readonly=True, required=False),
            'employee_id'   : fields.many2one('hr.employee', 'Driver', readonly=True, required=False),
            'office_id'     : fields.many2one('tms.office', 'Office', readonly=True, required=False),
    }
    ...
    def move_line_get_item(self, cr, uid, line, context=None):
        ...
        res.update({
            ...
            'office_id'     : line.office_id.id if line.office_id else False,
            'line_id'       : line.id,
