            ...
            <tr>
                ...
                <td>
                    <table style="width: 100%; text-align:center;">
                        ...
                        <tr><td><div class="td_company">Unidades: ${ data['vehicle_names'] or '' }</div></td></tr>
                    </table>
                </td>
            </tr>
        </table>
        ...
        <table class="list_table"  width="100%" border="0">
            <tbody>
                ...
                %if data['report_type'] == 'per':
                    <% 
                        info = ifrs.get_report_data( data['fiscalyear'], data['exchange_date'], data['currency_wizard'], data['target_move'], data['period'],two=False,context={'vehicle_ids':data['vehicle_ids']})
                ...
                %else:
                    <% 
                        info = ifrs.get_report_data( data['fiscalyear'], data['exchange_date'], data['currency_wizard'], data['target_move'], data['period'],two=True,context={'vehicle_ids':data['vehicle_ids']})
                    %>
                    %for ifrs_l in info:
                        <tr class="prueba">
                             %if not ifrs_l.get('invisible'):
